...
func (b *Broker) metadata(rw protocol.ResponseWriter, req *protocol.Request) error {
	...
	var getTopic func(string) (*store.Topic, protocol.ErrorCode)

	if len(r.Topics) > 0 {
		getTopic = func(name string) (*store.Topic, protocol.ErrorCode) {
			if validateTopicName(name) != nil {
				return nil, protocol.InvalidTopic
			} else {
				topic := b.Store.Topic(name)
				if topic != nil {
