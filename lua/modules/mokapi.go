...
func (m *Mokapi) on(l *lua.LState) int {
	...
	s := l.ToFunction(2)
	fn := func(ctx *common.EventContext) (bool, error) {
		values := make([]lua.LValue, 0, len(ctx.Args))
		...
		if retValues[0] == lua.LTrue {
			for i, val := range values {
				...
				if reflect.ValueOf(ctx.Args[i]).Kind() == reflect.Ptr {
					err = convert.FromLua(val, ctx.Args[i])
					if err != nil {
						return false, err
					}
				}
