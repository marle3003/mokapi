...
`,
			test: func(t *testing.T, c *openapi.Config) {
				require.NotNil(t, c)
				foo := c.Paths.Lookup("/foo").Value.Get.Responses.GetResponse(http.StatusOK).Content["application/json"].Schema
				require.NotNil(t, foo)
				require.NotNil(t, foo.Properties)
				require.Equal(t, json.Types{"object"}, foo.Properties.Get("content").Type)

				bar := c.Paths.Lookup("/bar").Value.Get.Responses.GetResponse(http.StatusOK).Content["application/json"].Schema
...
`,
			test: func(t *testing.T, c *openapi.Config) {
				require.NotNil(t, c)
				s := c.Paths.Lookup("/foo").Value.Get.Responses.GetResponse(http.StatusOK).Content["application/json"].Schema
				require.NotNil(t, s)
