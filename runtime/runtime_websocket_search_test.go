...
func TestIndex_Websocket(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			name: "Search by name",
			test: func(t *testing.T, app *runtime.App) {
				cfg := asyncapi3test.NewConfig(asyncapi3test.WithInfo("Websocket Test server", "", ""))
				_, err := app.Websocket.Add(toConfig(cfg), enginetest.NewEngine())
				...
				require.Equal(t,
					search.ResultItem{
						Type:      "Websocket",
						Title:     "Websocket Test server",
						Fragments: []string{"Websocket <mark>Test</mark> server"},
		...
		},
		{
			...
			test: func(t *testing.T, app *runtime.App) {
				...
				app.Websocket.Remove(toConfig(cfg))
				waitSearchIndex(t, func() bool {
					r, err = app.Search(search.Request{QueryText: "Test", Limit: 10})
					require.NoError(t, err)
		...
		},
		{
			...
			test: func(t *testing.T, app *runtime.App) {
				cfg := asyncapi3test.NewConfig(
					asyncapi3test.WithInfo("Websocket Test server", "", ""),
