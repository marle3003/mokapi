...
func TestParse_Enum(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			d:    "z",
			test: func(t *testing.T, v interface{}, err error) {
				require.EqualError(t, err, "value 'z' does not match one in the enumeration [a, b, c]")
			},
		},
		{
		...
		},
		{
			...
			d:    9,
			test: func(t *testing.T, v interface{}, err error) {
				require.EqualError(t, err, "value '9' does not match one in the enumeration [1, 2, 3]")
			},
		},
		{
		...
		},
		{
			...
			d:    1.5,
			test: func(t *testing.T, v interface{}, err error) {
				require.EqualError(t, err, "value '1.5' does not match one in the enumeration [1.1, 2.2, 3.3]")
			},
		},
		{
		...
		},
		{
			...
			test: func(t *testing.T, v interface{}, err error) {
				require.EqualError(t, err, "value '{foo: qux}' does not match one in the enumeration [{foo: bar}, {foo: baz}]")
			},
		},
		{
