...
func TestError_Marshal(t *testing.T) {
	testcases := []struct {
	...
	}{
		{
			...
			exp: `["string 'a' is less than minimum of 3"]`,
		},
		{
			name: "two error",
			err: Error{
			...
			exp: `["string 'a' is less than minimum of 3","item count 2 exceeds maximum count of 1"]`,
		},
		{
			name: "error detail",
			err: Error{
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.exp, Marshal(&tc.err))
		})
	}
}
