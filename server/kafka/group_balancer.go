...
type joindata struct {
	...
	writer           protocol.ResponseWriter
...
func (b *groupBalancerNew) run() {
	...
	for {
		...
		case s := <-b.sync:
			if b.group.State() == AwaitingSync {
				switch {
				case s.assigns != nil: // leader sync
					assigns = s.assigns
					syncs = append(syncs, s)
					b.group.SetState(Stable)
					for _, s := range syncs {
...
func (b *groupBalancerNew) respond(w protocol.ResponseWriter, msg protocol.Message) {
	go func() {
		err := w.Write(msg)
		if err != nil {
...
func newGroupAssignment(b []byte) *groupAssignment {
	g := &groupAssignment{}
	g.raw = b
	d := protocol.NewDecoder(bytes.NewReader(b), len(b))
