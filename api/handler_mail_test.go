...
func TestHandler_Smtp(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			responseBody: `[{"name":"foo","description":"bar","version":"1.0","type":"mail"}]`,
		},
		{
		...
		},
		{
			...
			contentType:  "application/json",
			responseBody: fmt.Sprintf(`{"name":"alice@foo.bar","numMessages":0,"folders":{"Inbox":{"mails":[{"from":[{"address":"bob@foo.bar"}],"to":[{"address":"alice@foo.bar"}],"messageId":"foo-1@mokapi.io","time":"%v","subject":"Hello Alice","contentType":"text/plain","body":"foobar"}]}}}`, now.Format(time.RFC3339Nano)),
		},
		{
			name: "get smtp mail",
			app: func() *runtime.App {
			...
			requestUrl:   "http://foo.api/api/services/mail/mails/foo-1@mokapi.io",
			contentType:  "application/json",
