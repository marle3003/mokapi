...
func readSearchRequest(p *ber.Packet) (*SearchRequest, error) {
	if len(p.Children) != 8 {
...
func (r *SearchRequest) toPacket() (*ber.Packet, error) {
...
func readSearchResponse(packets []*ber.Packet) (*SearchResponse, error) {
	res := &SearchResponse{}
	for _, p := range packets {
		if p.Tag == searchResult {
			r, err := readSearchResult(p)
			if err != nil {
				return nil, err
			}
...
func readSearchResult(p *ber.Packet) (SearchResult, error) {
...
func (r *SearchResponse) toPacket() []*ber.Packet {
	var packets []*ber.Packet
	for _, r := range r.Results {
		packets = append(packets, r.toPacket())
	}
	packets = append(packets, writeSearchDone(r))
	return packets
}

func (r *SearchResult) toPacket() *ber.Packet {
	...
	for k, v := range r.Attributes {
		...
		attrs.AppendChild(writeAttribute(k, v))
	}
...
func writeAttribute(name string, values []string) *ber.Packet {
...
func writeSearchDone(r *SearchResponse) *ber.Packet {
