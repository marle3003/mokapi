...
import WebsocketValidationErrors from "./WebsocketValidationErrors.vue";
...
watchEffect(() => {
  ...
  service.value = (result.service as Ref<WebsocketService | null>).value
  if (!service.value) {
    return null
  }
...
const message = computed(() => {
  ...
  return {source, contentType, contentTypeTitle: messageConfig.contentType}
})
...
<template>
  <div v-if="event && data && service">
          ...
          <div class="row mb-2">
            <div class="col-2">
              ...
              <p aria-labelledby="clientId">
                <router-link v-if="data.client && isClientAvailable(data.client.id)" :to="{
    ...
    <div class="card-group" v-if="message">
    ...
    <div class="card-group" v-else>
      <section class="card" aria-labelledby="value-title">
        <div class="card-body">
          <h2 id="value-title" class="card-title text-center">Value</h2>
          <source-view :source="{preview: {content: data.message.value!, contentType: 'application/json'}}" :hide-content-type="true" />
        </div>
      </section>
    </div>
  </div>
