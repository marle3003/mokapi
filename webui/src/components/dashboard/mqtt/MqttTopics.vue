...
function compareTopics(t1: MqttTopic, t2: MqttTopic) {
    const name1 = t1.name.toLowerCase()
    const name2 = t2.name.toLowerCase()
    return name1.localeCompare(name2)
}

function messages(service: Service, topic: MqttTopic) {
    return sum(service.metrics, 'mqtt_messages_total{', { name: 'topic', value: topic.name })
}

function lastMessage(service: Service, topic: MqttTopic) {
    const n = sum(service.metrics, 'mqtt_message_timestamp{', { name: 'topic', value: topic.name })
    if (n == 0) {
        return '-'
    }
    return format(n)
}
function goToTopic(topic: MqttTopic, openInNewTab = false) {
    if (getSelection()?.toString()) {
        return
    }

    const to = {
        name: getRouteName('mqttTopic').value,
        params: {
                        ...
                        <tr v-for="topic in topics" :key="topic.name" @mouseup.left="goToTopic(topic)"
                            ...
                            <td>
                                <router-link @click.stop class="row-link"
                                    :to="{ name: getRouteName('mqttTopic').value, params: { service: props.service.name, topic: topic.name } }">
                                    {{ topic.name }}
                                </router-link>
                            </td>
                            <td>
