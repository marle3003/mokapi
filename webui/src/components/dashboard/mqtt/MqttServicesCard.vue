...
function lastMessage(service: Service){
    const n = service.metrics.mqtt_message_timestamp
    if (n == 0){
        return '-'
    }
    return format(n)
...
<template>
    <section class="card" aria-labelledby="mqtt-clusters-title">
        <div class="card-body">
            <h2 class="card-title text-center" id="mqtt-clusters-title">MQTT Clusters</h2>
            <table class="table dataTable selectable" aria-labelledby="mqtt-clusters-title">
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="service in services" key="service.name" @mouseup.left="goToService(service)" @mousedown.middle="goToService(service, true)">
                        <td>
                            <router-link @click.stop class="row-link" :to="{ name: getRouteName('mqttService').value, params: {service: service.name} }">
                                {{ service.name }}
                            </router-link>
                        </td>
                        ...
                        <td class="text-center">{{ service.metrics.mqtt_messages_total }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
